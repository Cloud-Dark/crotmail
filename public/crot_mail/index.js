...
class SignJWT {
  ...
  setIssuedAt() {
    this.payload.iat = Math.floor(Date.now() / 1e3);
    return this;
  }
  setExpirationTime(exp) {
    if (typeof exp === "string" && exp.endsWith("m")) {
      this.payload.exp = Math.floor(Date.now() / 1e3) + parseInt(exp) * 60;
    } else if (typeof exp === "string" && exp.endsWith("h")) {
      this.payload.exp = Math.floor(Date.now() / 1e3) + parseInt(exp) * 3600;
    } else {
...
const mailboxById = /* @__PURE__ */ new Map();
const mailboxIdByAddress = /* @__PURE__ */ new Map();
const mailboxIdByResumeCode = /* @__PURE__ */ new Map();
const attachmentById = /* @__PURE__ */ new Map();
const streamSubscribersByMailboxId = /* @__PURE__ */ new Map();
const runtimeConfig = {
  accessKey: null,
  mailDomains: null,
  expireMinutes: null,
...
function getAvailableDomains(env) {
  ...
  return value.split(",").map((domain) => domain.trim().toLowerCase()).filter(Boolean);
}
...
async function issueAuthToken(env, { address, id, scope = "full", expiresInMinutes }) {
  ...
  return new SignJWT({ address, id, scope }).setProtectedHeader({ alg: "HS256" }).setIssuedAt().setExpirationTime(`${ttlMinutes}m`).sign(new TextEncoder().encode(secret));
}
...
async function createStream(request, env) {
  ...
  const pingTimer = setInterval(() => {
    sendSse(writer, encoder, "ping", { ts: (/* @__PURE__ */ new Date()).toISOString() }).catch(() => {
...
async function getTokenHandler(request, env) {
  ...
  const expireMinutes = getDefaultExpireMinutes(env);
  mailbox.expiresAt = new Date(Date.now() + expireMinutes * 60 * 1e3).toISOString();
  mailbox.updatedAt = (/* @__PURE__ */ new Date()).toISOString();
...
async function resumeByCode(request, env) {
  ...
  const remainingMinutes = Math.max(
    1,
    Math.floor((new Date(mailbox.expiresAt).getTime() - Date.now()) / 6e4)
  );
  const token = await issueAuthToken(env, {
    address: mailbox.address,
    id: mailbox.id,
...
async function extendExpiry(request, env) {
  ...
  const mailbox = mailboxById.get(user.id);
  mailbox.expiresAt = new Date(Date.now() + minutes * 60 * 1e3).toISOString();
  mailbox.updatedAt = (/* @__PURE__ */ new Date()).toISOString();
...
async function deleteMessage(request, env, id) {
  ...
  if (!user) return error("Unauthorized", 401);
  const mailbox = mailboxById.get(user.id);
  const index2 = mailbox.messages.findIndex((item) => item.id === id);
  if (index2 === -1) return error("Message not found", 404);
  const [message] = mailbox.messages.splice(index2, 1);
  for (const attachment of message.attachments || []) {
    attachmentById.delete(attachment.id);
  }
  return new Response(null, { status: 204 });
...
async function cleanupExpired(env) {
  const retentionMs = getMessageRetentionDays(env) * 24 * 60 * 60 * 1e3;
  const now = Date.now();
  ...
  async email(message, env) {
    ...
    const entry = {
      ...
      attachments: parsed.attachments.map((att) => {
        const id = generateId();
        const content = att.encoding === "base64" ? att.content.replace(/\s/g, "") : btoa(unescape(encodeURIComponent(att.content)));
        const attachment = {
          id,
